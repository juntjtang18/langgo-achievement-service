...
- unified event-bus payload:
  `topic = event name`, payload uses top-level `event_name`, `userid`, and `username`
- the service reads only `payload.event_name`, `payload.userid`, and `payload.username`
- `topic` is used only as a fallback when `event_name` is omitted
